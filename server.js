...
const resMime = require('./js/resMime.js'); //引入自定义模块，模块主要处理响应头
...
const server = http.createServer((req,res)=>{   //http.createServer()创建服务器
    ...
    if(pathName!= '/favicon.ico'){  
        ...
        //下面用到 fs 去读取客户端请求的文件，path.join拼接成绝对路径
         fs.readFile(path.join(__dirname,pathName), function(err,data){   //绝对路径，path.jion(__dirname,pathName)
         	if(err){    
...
server.listen('5555');    //让服务器跑在8887端口